...
local noYield = require(ReplicatedStorage.Utils.noYield)
local TableUtils = require(ReplicatedStorage.Utils.TableUtils)
local setInterval = require(ReplicatedStorage.Utils.setInterval)
local SessionLockedDataStoreWrapper = require(ReplicatedStorage.PlayerData.SessionLockedDataStoreWrapper)
...
local playerDataSaved = ReplicatedStorage.Remotes.PlayerDataSaved

local ALLOW_STUDIO_ACCESS = true
...
-- QUESTION what happen if we don't call SessionLockedDataStoreWrapper now? 
...
function PlayerDataServer.getValue(player: Player, valueName: string, syncedValueOnly: boolean?): any
	...
	local value
	-- If syncedValueOnly is passed, the value last loaded or saved to a DataStore successfully will be returned
...
-- Prepare player data or error for further send to client
function PlayerDataServer._onPlayerAddedAsync(player: Player)
	...
	if dataStoresEnabled then
		...
		if success then			
			...
			if GameConfig.resetData then
				local playerData = TableUtils.deepCopy(PlayerDataServer._defaultData) :: PlayerData
				PlayerDataServer._playerData[player] = playerData
			else
				local playerData = (result or TableUtils.deepCopy(PlayerDataServer._defaultData)) :: PlayerData
				PlayerDataServer._playerData[player] = playerData
			end
...
function PlayerDataServer._getKey(player: Player) : string
...
function PlayerDataServer._savePlayerDataAsync(player: Player, data: any, unlockAfter: boolean)
	...
	local success, result = PlayerDataServer._sessionLockedWrapper:setAsync(
		...
		data,
		{ tonumber(PlayerDataServer._getKey(player)) :: number },
		setOptions,
		{
